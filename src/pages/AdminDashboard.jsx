...
function SubmissionList({ type }) {
  ...
  return (
    <div className="space-y-4">
      <div className="overflow-x-auto rounded-2xl border border-navy/10 bg-white">
      <table className={`w-full ${isPartners ? "min-w-[1120px]" : "min-w-[760px]"} text-sm`}>
        <thead>
          <tr className="border-b border-navy/10 text-left text-xs font-semibold text-muted">
          ...
          {items.map((it) => (
            <tr key={it.id} className="border-b border-navy/5">
              ...
              {isPartners && (
                <td className="p-3 whitespace-nowrap">
                  <div className="flex flex-wrap gap-1.5">
                    <button disabled={updating} onClick={() => setSelectedPartner({ partner: it, editing: false })} className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-1 text-xs font-semibold text-navy hover:bg-slate-200 disabled:opacity-50"><Eye size={13} /> View</button>
...
function PartnerDetailsModal({ partner, editing, onClose, onEdit, onSaved }) {
  const [form, setForm] = useState({
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-navy/60 p-4">
      <form onSubmit={save} className="mx-auto my-5 max-w-4xl rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {PARTNER_FIELDS.map(([key, label, type]) => (
            <label key={key} className={`text-xs font-semibold text-navy/70 ${type === "textarea" ? "sm:col-span-2" : ""}`}>
              {label}
              {!editing ? <span className="mt-1 block rounded-lg bg-slate-50 px-3 py-2 text-sm font-normal text-navy">{fieldValue(key)}</span> : type === "select" ? (
        ...
        <div className="mt-5 border-t border-navy/10 pt-4">
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {PARTNER_DOCUMENTS.map(([key, label, urlKey]) => (
